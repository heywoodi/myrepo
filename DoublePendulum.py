...
class DP():
    ...
    def updCartesian(self):
        ...
        vy1 = self.w1 * self.l1 * math.sin(self.a1)
        vx2 = vx1 + self.w2 * self.l2 * math.cos(self.a2)
        vy2 = vy1 + self.w2 * self.l2 * math.sin(self.a2)
        self.v1 = [vx1,vy1]
        self.v2 = [vx2,vy2]
    ...
    def updEnergies(self):
        ...
        self.totalE = self.PE1 + self.KE1 + self.PE2 + self.KE2

    def updMomentum(self):
        ''' updates the momentum of the pendulum masses '''
        self.p1 = [ self.m1 * self.v1[0] , self.m1 * self.v1[1] ] 
        self.p2 = [ self.m2 * self.v2[0] , self.m2 * self.v2[1] ]
    ...
    def updEuler(self, deltaT):
        ...
        self.a1 += self.w1*deltaT
        self.a2 += self.w2*deltaT

        self.w1 += aa1 * deltaT
        self.w2 += aa2 * deltaT


    def updEulerCromer(self, deltaT):   #Numerical integration based on Euler Cromer method
    ...
    def RK4XDashArray(self, w1, w2, theta1, theta2):
        ...
        theta1dash = w1
        theta2dash = w2
        return np.array([w1dash, w2dash, theta1dash, theta2dash])
    ...
    def updRungeKutta(self, h):
        ...
        Xn = np.array([self.w1, self.w2, self.a1, self.a2])
